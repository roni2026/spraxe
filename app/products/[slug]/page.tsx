...
async function fetchProductUncached(slug: string): Promise<ProductRow | null> {
  ...
  const res: { data: ProductRow | null; error: any } = await supabase
    .from('products')
    .select(
      'id,category_id,name,slug,description,sku,images,price,base_price,retail_price,stock_quantity,unit,supplier_name,tags,is_active,size_chart,color_group_id,color_name,color_hex,is_featured,total_sales,approval_status,seo_title,seo_description,seo_keywords'
    )
    .eq('slug', slug)
    .eq('is_active', true)
    .maybeSingle();
...
export async function generateMetadata({
  ...
  const autoDesc = stripHtml(String(product.description || '')).slice(0, 160);
  const title = (product.seo_title || '').trim() || product.name;
  const desc = (product.seo_description || '').trim() || autoDesc || `Buy ${product.name} at Spraxe Bangladesh.`;
