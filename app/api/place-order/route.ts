  ...
  try {
    ...
    const { data: order, error: orderErr } = await supabaseAdmin
      ...
      .insert({
        ...
        notes: `Shipping: ${shippingSpeed === 'express' ? 'Express' : 'Standard'} • Area: ${deliveryLocation === 'inside' ? 'Inside Dhaka' : 'Outside Dhaka'}${body.orderNote ? ` • Customer note: ${String(body.orderNote).slice(0, 500)}` : ''}`,
