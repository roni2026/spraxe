...
// The login session lives in the browser (localStorage), so the server cannot
// see it from cookies. We must send the access token ourselves so the sign /
// delete endpoints can verify who is asking (same pattern as /api/place-order).
