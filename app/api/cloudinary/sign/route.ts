...
export async function POST(req: Request) {
  ...
  // Require a logged-in user. The browser session lives in localStorage, so a
  // fresh server client never sees it — the client must send its access token
  // as a Bearer header and we validate that token explicitly here (same
  // pattern as /api/place-order).
  try {
